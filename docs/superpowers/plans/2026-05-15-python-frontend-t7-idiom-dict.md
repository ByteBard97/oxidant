# Task 7: Python Idiom Dictionary

**Part of:** [Python→Rust Frontend](2026-05-15-python-frontend-overview.md) — do Task 5 first (it wires `_load_idiom_entries` to pick this file).

**Goal:** Create `idiom_dictionary_python.md` at the workspace root — the Python→Rust guidance file injected into Phase B prompts. Placed at the same level as the existing `idiom_dictionary.md` for TypeScript.

**Files:**
- Create: `idiom_dictionary_python.md` (at `/Users/ceres/Desktop/SignalCanvas/oxidant/idiom_dictionary_python.md`)

---

- [ ] **Step 1: Create `idiom_dictionary_python.md`**
...
- [ ] **Step 2: Verify the idiom loader can parse it**
...
import re
content = Path('idiom_dictionary_python.md').read_text()
for idiom in ['list_comprehension', 'optional_type', 'format_string', 'none_check', 'multiple_return']:
    pattern = re.compile(rf'^##\s+{re.escape(idiom)}\b.*?(?=^##\s|\Z)', re.MULTILINE | re.DOTALL)
    m = pattern.search(content)
    assert m, f'{idiom} section not found'
    print(f'OK: {idiom}')
"
```
...
```bash
git add idiom_dictionary_python.md
git commit -m "docs: Python idiom dictionary for Phase B prompts"
```

**Next:** [Task 8 — End-to-end test](2026-05-15-python-frontend-t8-e2e-test.md)
